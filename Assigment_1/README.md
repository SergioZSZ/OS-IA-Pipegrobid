...
Para realizar las siguientes validaciones se usó el documento `.pdf` con ruta `/Assigment_1/pdfs/A Multi-Agent LLM Framework for Realistic Patient.pdf`

### 1. Funcionamiento de GROBID

En el propio Script `/Assigment_1/main.py` se codificó una peticion GET a la API de GROBID para verificar su funcionamiento con un timeout de 5 segundos. Pueden ocurrir 2 casos:
...
Para verificar la Coherencia de los datos extraídos, se añadieron en el Script `/Assigment_1/functions/process_xml.py` en la funcion `process_xml()` dos bloques de código:

- 1º (líneas 71-82): Para el primer paper generado (correspondiente al pdf mencionado al principio de este bloque) se imprime por pantalla el nombre del paper, el abstract extraído, su número de figuras y links (posteriormente añadidas al diccionario que se envía a `/Assigment_1/main.py`), Saliendo por pantalla:
```
***************************** VALIDACION ***************************** 
...
Yéndonos a las líneas 76-78 del paper correspondiente `/Assigment_1/xmls/paper1.tei.xml` encontramos el abstract con el mismo texto que sale por pantalla:
```
			<abstract>
...
Por pantalla se muestra nfigures: 6 (al igual que en `/Assigment_1/generated_files/figures_visualization.png` sale que el paper1 tiene 6 figuras)
En el mismo `/Assigment_1/xmls/paper1.tei.xml` en las líneas 108, 109, 110, 111, 112 y 113 encontramos las 6 figuras correspondientes, sin encontrar más en el paper
